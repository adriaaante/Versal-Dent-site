...
> **ОБНОВЛЕНО 22.06.2026 (подтверждено владельцем).** Большая часть
> заглушек ниже уже закрыта реальными данными — список оставлен как
> история. Актуальный статус:
> - ✅ **Контакты, адрес, карта** — реальные. Координаты
>   `ll=37.849032,55.766978`, `oid=107897418441` указывают на реальный
>   адрес в Реутове (НЕ «центр Москвы» — старое замечание неверно).
> - ✅ **Реквизиты/лицензия** — реальные. Обе клиники владельца (Ангел и
>   Версаль) работают под одним юрлицом **ООО «АНГЕЛ-ДЕНТ»** (ИНН
>   5012077543, ОГРН 1135012000936, лицензия ЛО41-01162-50/00299266),
>   ген. директор Геворкян С.И., главврач Дробкова К.О. — это НЕ чужие
>   реквизиты, а оператор обеих клиник.
> - ✅ **ФИО врачей** заполнены на всех страницах; **Яндекс.Метрика** —
>   реальный id `109728396`; Telegram/WhatsApp — реальные.
> - ✅ **`sitemap.xml`** — 22.06.2026 заменены 9 плейсхолдеров
>   `[ФИО врача]` в `<image:title>` на реальные имена и фото (синхрон со
>   страницами). **Требует деплоя** `~/versal-dent.sh`.
> - 🔧 **Ещё открыто:** фото Киласонии (`placeholder.svg`); образование/
>   сертификаты врачей (`<!-- TODO -->`); возврат `aggregateRating` при
>   появлении реальных отзывов; VK/Дзен (соцсети пока только TG/WA).
...
- **Город / адрес / индекс / метро:** `Реутове`, `ул. Победы, д. 22`,
  `143968`, `Новокосино`, `Новокосино`, `Московская область`, `у м. Новокосино`
- **Координаты карты:** в `index.html` и `contacts.html` карта
  Яндекса стоит на placeholder-координатах (центр Москвы) — заменить
  `ll=37.849032%2C55.766978&z=17&mode=search&oid=107897418441&ol=biz` на реальные `ll`/`oid` клиники.
- **Реквизиты:** `1135012000936`, `5012077543`, `ЛО41-01162-50/00299266 от 19.07.2016`,
  `Московская область, г. Красногорск, бульвар Строителей, д. 1`, ФИО ген. директора в `oferta.html`.
...
- **Email:** `versal.dent@mail.ru` (подтвердить).
- **Яндекс.Метрика:** счётчик-placeholder `00000000` в `<head>` каждой
  страницы — вписать реальный id (см. ниже).
- **Telegram-бот заявок:** `api/config.php` — завести на сервере из
  `api/config.php.example` (`cp` + вписать токен бота). `chat_id` уже
  задан в шаблоне — группа заявок **-5172196362**. Бота добавить в группу
  с правом отправки сообщений. Поток: форма → `assets/js/main.js`
  (`sendLead`) → `api/lead.php` → Telegram API.
